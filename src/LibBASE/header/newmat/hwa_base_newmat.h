...
#define BandLUMatrix BLUMatrix
    ...
    // Somewhat implementation dependent as it relies on "std::cout" still being
    ...
    public:
        ...
        enum
        {
            ...
            Dg = Valid + Diagonal + Band + Lower + Upper + Symmetric + Square,
            Id = Valid + Diagonal + Band + Lower + Upper + Symmetric + Square + Ones,
            RV = Valid, //   do not separate out
            CV = Valid, //   std::vectors
    ...
    public:
        ...
        const char *value() const; ///< type as char std::string
    ...
    class LinearEBASE_QUATionSolver;
    class GenericMatrix;
...
#define MatrixTypeUnSp 0
    ...
    public:
        ...
        friend class LinearEBASE_QUATionSolver;
    ...
    protected:
        ...
        Real *store;                           // point to store (0=not std::set)
        ...
        void operator=(Real);                  // std::set matrix to constant
    ...
    public:
        ...
        virtual ~GeneralMatrix();        // delete store if std::set
        ...
        // std::set parameters in GetMatrix
    ...
    public:
        ...
        Real *operator[](int m)
        {
            return store + (int64_t)m * ncols_val;
        }
...
#endif
        Matrix(const Matrix &gm) : GeneralMatrix()
        {
            GetMatrix(&gm);
        }
        ...
        virtual void resize(int, int); // change dimensions
                                       // virtual so we will catch it being used in a std::vector called as a matrix
    ...
    public:
        ...
        void operator=(const Eigen::VectorXd& m) {
            const int n = m.size();
            resize(n, 1);
            for (int i = 0; i < n; ++i)
                store[i+1] = m[i];
        }
        void operator=(const Eigen::Vector3d& m) {
            resize(3, 1);
            for (int i = 0; i < 3; ++i)
                store[i + 1] = m[i];
        }
        Eigen::VectorXd toEV() {
            Eigen::VectorXd tmp(storage);
            for (int i = 0; i < storage; ++i)
                tmp[i] = store[i+1];
        }
        void unitary() {
            double s = this->norm_Frobenius();
            *this /= s; 
        }
        Real &operator()(int);      // access element
        Real &element(int);         // access element
    ...
    class BandMatrix : public GeneralMatrix
    {
        GeneralMatrix *Image() const; // copy of matrix
    protected:
        void CornerClear() const; // std::set unused elements to zero
    ...
    public:
...
#endif
        BandMatrix(const BandMatrix &gm) : GeneralMatrix()
        {
            GetMatrix(&gm);
        }
        LogAndSign log_determinant() const;
        ...
        {
            CornerClear();
            return GeneralMatrix::sum_square();
        }
        Real sum_absolute_value() const
        {
            CornerClear();
        ...
        {
            CornerClear();
            return GeneralMatrix::sum();
        }
        Real maximum_absolute_value() const
        {
            CornerClear();
            return GeneralMatrix::maximum_absolute_value();
        }
        Real minimum_absolute_value() const
        {
            int i, j;
            return GeneralMatrix::minimum_absolute_value2(i, j);
        }
        Real maximum() const
        {
            int i, j;
            return GeneralMatrix::maximum2(i, j);
        }
        Real minimum() const
        {
            int i, j;
            return GeneralMatrix::minimum2(i, j);
        }
        void GetRow(MatrixRowCol &);
        void GetCol(MatrixRowCol &);
        void GetCol(MatrixColX &);
    ...
    public:
        ...
        Real *operator[](int m)
        {
            return store + upper_val * (int64_t)m;
        }
    ...
    public:
        ...
        Real *operator[](int m)
        {
            return store + lower_val * ((int64_t)m + 1);
        }
    ...
    class LinearEBASE_QUATionSolver : public BaseMatrix
    {
        GeneralMatrix *gm;
    ...
    public:
        LinearEBASE_QUATionSolver(const BaseMatrix &bm);
        ~LinearEBASE_QUATionSolver() { delete gm; }
        ...
        NEW_DELETE(LinearEBASE_QUATionSolver)
    };
    ...
    public:
        ...
        void operator=(int ai); ///< std::set the array equal to a constant
        ...
        void cleanup() { resize(0); } ///< std::set length to zero
        void CleanUp() { resize(0); } ///< std::set length to zero
    ...
    /// Cannot convert to std::vector exception.
    ...
    inline Real norm_Frobenius(const BaseMatrix &B) { return B.norm(); }
    inline Real norm_frobenius(const BaseMatrix &B) { return B.norm(); }
    inline Real NormFrobenius(const BaseMatrix &B) { return B.norm(); }
    ...
    inline Real Norm2(const ColumnVector &CV)
    {
        return CV.norm();
    }
