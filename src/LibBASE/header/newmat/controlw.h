...
/// Organise an int as a series of bits to std::set options.
/// \internal
class ControlWord
{
protected:
...
public:
    ...
    // select specific bits (for testing at least one std::set)
    ...
    // check if all of selected bits std::set or reset
