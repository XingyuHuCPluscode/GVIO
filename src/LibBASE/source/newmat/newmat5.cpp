...
#define REPORT                                   \
    {                                            \
        static ExeCounter ExeCount(__LINE__, 5); \
        ++ExeCount;                              \
    }
#else
#define REPORT \
    ...
    {
        REPORT
        GeneralMatrix *gmx = new RowVector;
        MatrixErrorNoSpace(gmx);
        gmx->nrows_val = 1;
        gmx->ncols_val = gmx->storage = storage;
    ...
    {
        REPORT
        GeneralMatrix *gmx = new ColumnVector;
        MatrixErrorNoSpace(gmx);
        gmx->ncols_val = 1;
        gmx->nrows_val = gmx->storage = storage;
        return BorrowStore(gmx, mt);
    }
    ...
    GeneralMatrix *CroutMatrix::Evaluate(MatrixType mt)
    {
        ...
        bool dummy = true;
        if (dummy)
            Throw(ProgramException("Illegal use of CroutMatrix", *this));
        return this;
    }
    ...
    GeneralMatrix *RowedMatrix::Evaluate(MatrixType mt)
    {
        ...
        MatrixErrorNoSpace(gmx);
        gmx->nrows_val = 1;
        gmx->ncols_val = gmx->storage = gm->storage;
    ...
    GeneralMatrix *ColedMatrix::Evaluate(MatrixType mt)
    {
        ...
        MatrixErrorNoSpace(gmx);
        gmx->ncols_val = 1;
        gmx->nrows_val = gmx->storage = gm->storage;
        return gm->BorrowStore(gmx, mt);
    }
    ...
    {
        gm = ((BaseMatrix *&)bm)->Evaluate();
        GeneralMatrix *gmx = new DiagonalMatrix;
        MatrixErrorNoSpace(gmx);
    ...
    GeneralMatrix *MatedMatrix::Evaluate(MatrixType mt)
    {
        ...
        gm = ((BaseMatrix *&)bm)->Evaluate();
        GeneralMatrix *gmx = new Matrix;
        MatrixErrorNoSpace(gmx);
        gmx->nrows_val = nr;
        gmx->ncols_val = nc;
        gmx->storage = gm->storage;
    ...
    MatrixInput MatrixInput::operator<<(double f)
    {
        ...
        if (n <= 0)
            Throw(ProgramException("List of values too long"));
        *r = (Real)f;
        int n1 = n - 1;
    ...
    MatrixInput GeneralMatrix::operator<<(double f)
    {
        ...
        int n = Storage();
        if (n <= 0)
            Throw(ProgramException("Loading data to zero length matrix"));
        Real *r;
        r = Store();
        *r = (Real)f;
        n--;
