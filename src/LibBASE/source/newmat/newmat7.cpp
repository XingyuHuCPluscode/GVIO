...
#define REPORT                                   \
    {                                            \
        static ExeCounter ExeCount(__LINE__, 7); \
        ++ExeCount;                              \
    }
#else
#define REPORT \
                                       ...
                                       const MatrixColX &mcin)
    {
        ...
        while (j-- > 0)
            *elx++ = 0.0;
        Real *Ael = store + (nr * (2 * ncols_val - nr + 1)) / 2;
        j = 0;
        while (i-- > 0)
        {
            elx = el;
                                       ...
                                       MultipliedMatrix *mm, MatrixType mtx)
    {
        ...
        // or column std::vectors in right hand term.
        REPORT
                                      ...
                                      MultipliedMatrix *mm, MatrixType mtx)
    {
        ...
        if (nc <= 5 && nr > nc)
        {
            REPORT return GeneralMult1(gm1, gm2, mm, mtx);
        }
                                      ...
                                      BaseMatrix *sm, MatrixType mtx)
    {
        ...
        int nr = gm1->Nrows();
        if (nr != gm1->Ncols())
            Throw(NotSquareException(*gm1));
        int nc = gm2->Ncols();
        if (gm1->Ncols() != gm2->Nrows())
            Throw(IncompatibleDimensionsException(*gm1, *gm2));
        GeneralMatrix *gmx = mtx.New(nr, nc, sm);
        MatrixErrorNoSpace(gmx);
        Real *r = new Real[nr];
        MatrixErrorNoSpace(r);
    ...
    GeneralMatrix *AddedMatrix::Evaluate(MatrixType mtd)
    {
        ...
        if (c1 && c2 && (gm1->SimpleAddOK(gm2) == 0))
        {
            if (gm1->reuse())
            {
                REPORT AddTo(gm1, gm2);
                gm2->tDelete();
                gmx = gm1;
            }
            else if (gm2->reuse())
            {
                REPORT AddTo(gm2, gm1);
                gmx = gm2;
            }
            else
            {
    ...
    GeneralMatrix *SubtractedMatrix::Evaluate(MatrixType mtd)
    {
        ...
        if (c1 && c2 && (gm1->SimpleAddOK(gm2) == 0))
        {
            if (gm1->reuse())
            {
                REPORT SubtractFrom(gm1, gm2);
                gm2->tDelete();
                gmx = gm1;
            }
            else if (gm2->reuse())
            {
                REPORT ReverseSubtract(gm2, gm1);
                gmx = gm2;
            }
            else
            {
    ...
    bool operator==(const BaseMatrix &A, const BaseMatrix &B)
    {
        ...
        MatrixType AType = gmA->type();
        MatrixType BType = gmB->type();
    ...
    bool operator==(const GeneralMatrix &A, const GeneralMatrix &B)
    {
        ...
        // check for CroutMatrix or BandLUMatrix
        MatrixType AType = A.Type();
        MatrixType BType = B.Type();
        if (AType.CannotConvert() || BType.CannotConvert())
        {
    ...
    bool CroutMatrix::IsEqual(const GeneralMatrix &A) const
    {
        ...
        // is matrix storage the same - compare store
        REPORT
        return RealEqual(A.store, store, storage) && intEqual(((CroutMatrix &)A).indx, indx, nrows_val);
    }
    ...
    bool BandLUMatrix::IsEqual(const GeneralMatrix &A) const
    {
        ...
        if (A.Nrows() != nrows_val || A.Ncols() != ncols_val || ((BandLUMatrix &)A).m1 != m1 || ((BandLUMatrix &)A).m2 != m2)
        // different dimensions
        {
            REPORT return false;
        }
        ...
        REPORT
        return RealEqual(A.Store(), store, storage) && RealEqual(((BandLUMatrix &)A).store2, store2, storage2) && intEqual(((BandLUMatrix &)A).indx, indx, nrows_val);
    }
