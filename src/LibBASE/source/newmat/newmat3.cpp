...
#define REPORT                                   \
    {                                            \
        static ExeCounter ExeCount(__LINE__, 3); \
        ++ExeCount;                              \
    }
#else
#define REPORT \
    ...
    //   { std::cout << what << " " << storage << " at " << (long)store << "\n"; }
...
#define MONITOR(what, store, storage) \
    ...
    // DirectPart:
    //    Load or restore only part directly stored; must be std::set with StoreOnExit
    ...
    // For symmetric matrices, treat columns as rows unless StoreHere is std::set;
    ...
    void GeneralMatrix::NextRow(MatrixRowCol &mrc)
    {
        ...
        {
            REPORT this->RestoreRow(mrc);
        }
        mrc.rowcol++;
        if (mrc.rowcol < nrows_val)
        {
            REPORT this->GetRow(mrc);
        }
        else
        {
            REPORT mrc.cw -= StoreOnExit;
        }
    }
    ...
    void GeneralMatrix::NextCol(MatrixRowCol &mrc)
    {
        REPORT // 423
            if (+(mrc.cw * StoreOnExit))
        {
            REPORT this->RestoreCol(mrc);
        }
        mrc.rowcol++;
        if (mrc.rowcol < ncols_val)
        {
            REPORT this->GetCol(mrc);
        }
        else
        {
            REPORT mrc.cw -= StoreOnExit;
    ...
    void Matrix::GetRow(MatrixRowCol &mrc)
    {
        REPORT
        mrc.skip = 0;
        mrc.storage = mrc.length = ncols_val;
    ...
    void Matrix::GetCol(MatrixRowCol &mrc)
    {
        REPORT
        mrc.skip = 0;
        mrc.storage = mrc.length = nrows_val;
        ...
        else
        {
            ...
            {
                REPORT
                ColCopy = new Real[nrows_val];
                MatrixErrorNoSpace(ColCopy);
    ...
    void DiagonalMatrix::GetCol(MatrixRowCol &mrc)
    {
        ...
        {
            REPORT mrc.data = store + mrc.skip;
        }
        // not accessed
    }
    ...
    void UpperTriangularMatrix::GetRow(MatrixRowCol &mrc)
    {
        ...
        mrc.skip = row;
        mrc.length = ncols_val;
        mrc.storage = ncols_val - row;
        mrc.data = store + (row * (2 * ncols_val - row + 1)) / 2;
    }
    ...
    void SymmetricMatrix::GetRow(MatrixRowCol &mrc)
    {
        ...
        else
        {
            ...
            mrc.storage = ncols_val;
            Real *RowCopy;
            if (!(mrc.cw * HaveStore))
            {
                REPORT
                RowCopy = new Real[ncols_val];
                MatrixErrorNoSpace(RowCopy);
            ...
            {
                REPORT RowCopy = mrc.data;
            }
            if (+(mrc.cw * LoadOnEntry))
            {
                REPORT // 544
                    Real *Mstore = store + (row * (row + 1)) / 2;
                int i = row;
                while (i--)
                    *RowCopy++ = *Mstore++;
                i = ncols_val - row;
    ...
    void SymmetricMatrix::GetCol(MatrixRowCol &mrc)
    {
        ...
        {
            REPORT mrc.storage = col + 1;
            mrc.data = store + (col * (col + 1)) / 2;
        }
        else
        {
            // do not allow StoreOnExit and !DirectPart
    ...
    void BandMatrix::GetRow(MatrixRowCol &mrc)
    {
        REPORT
        int r = mrc.rowcol;
        int w = lower_val + 1 + upper_val;
        mrc.length = ncols_val;
        int s = r - lower_val;
        if (s < 0)
        {
            mrc.data = store + (r * w - s);
            w += s;
            s = 0;
        }
        else
    ...
    void BandMatrix::NextRow(MatrixRowCol &mrc)
    {
        ...
        {
            mrc.storage++;
            mrc.data += lower_val + upper_val;
        }
        else
        {
            mrc.skip++;
            mrc.data += lower_val + upper_val + 1;
        }
        if (r >= ncols_val - upper_val)
            mrc.storage--;
    }
    ...
    void SymmetricBandMatrix::GetRow(MatrixRowCol &mrc)
    {
        ...
        else
        {
            ...
            {
                REPORT
                RowCopy = new Real[2 * lower_val + 1];
                MatrixErrorNoSpace(RowCopy);
    ...
    void SymmetricBandMatrix::GetCol(MatrixRowCol &mrc)
    {
        ...
        else
        {
            ...
            {
                REPORT ColCopy = new Real[2 * lower_val + 1];
                MatrixErrorNoSpace(ColCopy);
