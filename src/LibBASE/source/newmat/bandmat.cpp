...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 10); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
    ...
    void BandMatrix::resize(int n, int lb, int ub)
    {
        ...
        if (lb < 0 || ub < 0)
            Throw(ProgramException("Undefined bandwidth"));
        lower_val = (lb <= n) ? lb : n - 1;
        upper_val = (ub <= n) ? ub : n - 1;
        GeneralMatrix::resize(n, n, n * (lower_val + 1 + upper_val));
        CornerClear();
    }

    // SimpleAddOK shows when we can add etc two matrices by a simple std::vector add
    ...
    /// \brief can we add two band matrices with simple std::vector add
    ///
    ...
    /// \brief can we add two symmetric band matrices with simple std::vector add
    ///
    ...
    short SymmetricBandMatrix::SimpleAddOK(const GeneralMatrix *gm)
    {
        ...
        if (bm->lower_val == lower_val)
        {
            REPORT return 0;
        }
        else if (bm->lower_val > lower_val)
        {
            REPORT return 1;
        }
        else
...
*/
    ...
    /// \brief std::set unused parts of BandMatrix to zero
    ...
    MatrixBandWidth MatrixBandWidth::operator+(const MatrixBandWidth &bw) const
    {
        ...
        int l = bw.lower_val;
        int u = bw.upper_val;
        l = (lower_val < 0 || l < 0) ? -1 : (lower_val > l) ? lower_val
                                                            : l;
        u = (upper_val < 0 || u < 0) ? -1 : (upper_val > u) ? upper_val
                                                            : u;
    ...
    MatrixBandWidth MatrixBandWidth::minimum(const MatrixBandWidth &bw) const
    {
        ...
        int l = bw.lower_val;
        int u = bw.upper_val;
        if ((lower_val >= 0) && ((l < 0) || (l > lower_val)))
            l = lower_val;
        if ((upper_val >= 0) && ((u < 0) || (u > upper_val)))
            u = upper_val;
        return MatrixBandWidth(l, u);
    }
    ...
    BandLUMatrix::BandLUMatrix(const BaseMatrix &m)
    {
        ...
        storage2 = 0;
        store2 = 0;
        indx = 0; // in event of exception during build
        ...
        {
            REPORT((BandLUMatrix *)gm)->get_aux(*this);
            GetMatrix(gm);
        }
        else
        {
            ...
            store2 = new Real[storage2];
            MatrixErrorNoSpace(store2);
    ...
    GeneralMatrix *BandLUMatrix::Evaluate(MatrixType mt)
    {
        ...
        bool dummy = true;
        if (dummy)
            Throw(ProgramException("Illegal use of BandLUMatrix", *this));
        return this;
    }
    ...
    void BandLUMatrix::Solver(MatrixColX &mcout, const MatrixColX &mcin)
    {
        ...
        while (i--)
            *el++ = 0.0;
        el += mcin.storage;
        i = nrows_val - mcin.skip - mcin.storage;
        while (i--)
            *el++ = 0.0;
    ...
    {
        REPORT
        int i = mcin.skip - mcout.skip;
        Real *elx = mcin.data - i;
        while (i-- > 0)
            *elx++ = 0.0;
        int nr = mcin.skip + mcin.storage;
        elx = mcin.data + mcin.storage;
        Real *el = elx;
        ...
        Real *Ael = store + (upper_val + 1) * (i - 1) + 1;
        j = 0;
        if (i > 0)
            for (;;)
            {
                                 ...
                                 const MatrixColX &mcin)
    {
        ...
        i = nc + mcin.storage;
        elx = mcin.data + mcin.storage;
        int nr = mcout.skip + mcout.storage;
        int j = nr - i;
        i = nr - nc;
        while (j-- > 0)
            *elx++ = 0.0;
...
*/
    ...
    {
        // std::set unused parts of BandMatrix to zero
        REPORT
        int i = lower_val;
        Real *s = store;
        int bw = lower_val + 1;
    ...
    {
        REPORT
        GeneralMatrix *gm = new BandMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new UpperBandMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new LowerBandMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new SymmetricBandMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
