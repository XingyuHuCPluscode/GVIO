...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 15); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
...
#include <iostream>
    ...
    int SVD(const Matrix &A, DiagonalMatrix &Q, Matrix &U, Matrix &V,
    ...
    // from Wilkinson and Reinsch: "Handbook of Automatic Computation"
    {
        ...
        if (n)
            for (i = 0;;)
            {
                ...
                Real ei = g;
                EI.Right();
                Real s = UCI.SumSquare();
                if (s < tol)
                {
                    REPORT Q.element(i) = 0.0;
                }
                else
                {
                    REPORT
                ...
                Real y = fabs(Q.element(i)) + fabs(ei);
                if (x < y)
                {
                    REPORT x = y;
                }
        ...
        for (int k = n - 1; k >= 0; k--)
        {
            ...
            while (limit--)
            {
                ...
                for (l = k; l >= 0; l--)
                {
                    ...
                    if (fabs(E.element(l)) <= eps)
                    {
                        REPORT tfc = true;
                        break;
                    }
                    if (fabs(Q.element(l - 1)) <= eps)
                    {
                        REPORT l1 = l;
                        break;
                    }
                ...
                for (i = l + 1; i <= k; i++)
                {
                    ...
                    {
                        REPORT
                        RectMatrixCol VCI(V, i);
                        RectMatrixCol VCJ(V, i - 1);
