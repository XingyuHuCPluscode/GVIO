...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 11); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
    ...
    void GetSubMatrix::SetUpLHS()
    {
        ...
        if (row_number < 0)
            row_number = gm1->Nrows();
        if (col_number < 0)
            col_number = gm1->Ncols();
        if (row_skip + row_number > gm1->Nrows() || col_skip + col_number > gm1->Ncols())
            Throw(SubMatrixDimensionException());
    }
    ...
    void GetSubMatrix::operator=(const BaseMatrix &bmx)
    {
        ...
        Try
        {
            ...
            LoadAndStoreFlag lasf =
                (row_skip == col_skip && gm->type().is_symmetric() && gmx->type().is_symmetric())
