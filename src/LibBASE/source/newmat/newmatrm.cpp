...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 12); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
    ...
    {
        REPORT
        RectMatrixRowCol::Reset(M.Store() + row * M.Ncols() + skip, length, 1, M.Ncols());
    }
    ...
    {
        REPORT
        RectMatrixRowCol::Reset(M.Store() + col + skip * M.Ncols(), length, M.Ncols(), 1);
    }
