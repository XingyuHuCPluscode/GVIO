...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 18); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
                ...
                Matrix &V, bool eivec)
    {
        ...
        for (int i = 1; i <= 50; i++)
        {
            ...
            int p = A.Storage();
            while (p--)
                sm += fabs(*a++); // have previously zeroed diags
            if (sm == 0.0)
            {
                REPORT converged = true;
                break;
            }
            Real tresh = (i < 4) ? 0.2 * sm / square(n) : 0.0;
            a = A.Store();
            for (p = 0; p < n; p++)
            {
                Real *ap1 = a + (p * (p + 1)) / 2;
                ...
                for (int q = p + 1; q < n; q++)
                {
                    ...
                    if (i > 4 && g < epsilon * adp && g < epsilon * adq)
                    {
                        REPORT apq = 0.0;
                    }
                    else if (fabs(apq) > tresh)
                    {
                        ...
                        {
                            REPORT
                            RectMatrixCol VP(V, p);
                            RectMatrixCol VQ(V, q);
                            Rotate(VP, VQ, tau, s);
                        }
                    }
                }
    ...
    {
        REPORT SymmetricMatrix A;
        Matrix V;
        Jacobi(X, D, A, V, false);
    }
