...
#define REPORT                                   \
    {                                            \
        static ExeCounter ExeCount(__LINE__, 6); \
        ++ExeCount;                              \
    }
#else
#define REPORT \
    ...
    void GenericMatrix::operator+=(const BaseMatrix &X)
    {
        ...
        if (!gm)
            Throw(ProgramException("GenericMatrix is null"));
    ...
    Real &DiagonalMatrix::element(int m)
    {
        REPORT
        if (m < 0 || m >= nrows_val)
            Throw(IndexException(m, *this, true));
        return store[m];
    }
    ...
    Real &RowVector::element(int n)
    {
        REPORT
        if (n < 0 || n >= ncols_val)
            Throw(IndexException(n, *this, true));
        return store[n];
    }
