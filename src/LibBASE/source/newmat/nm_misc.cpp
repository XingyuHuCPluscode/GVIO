...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 21); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
    ...
    ReturnMatrix Helmert(int n, bool full)
    {
        ...
        Tracer et("Helmert ");
        if (n <= 0)
            Throw(ProgramException("Dimension <= 0 "));
        Matrix H;
        ...
        for (int i = 1; i < n; ++i)
        {
            ...
            H.submatrix(i, i, 1, i) = -f;
            H(i, i + 1) = f * i;
        }
        if (full)
        {
            H.row(n) = 1.0 / sqrt((Real)n);
        }
        H.release();
        return H.for_return();
    }
    ...
    ReturnMatrix Helmert(const ColumnVector &X, bool full)
    {
        ...
        int n = X.nrows();
        if (n == 0)
            Throw(ProgramException("X Vector of length 0", X));
        Real sum = 0.0;
        ColumnVector Y;
        if (full)
            Y.resize(n);
        ...
        {
            sum += X(i);
            Y(i) = (i * X(i + 1) - sum) / sqrt((Real)i * (i + 1));
        }
        if (full)
        {
            sum += X(n);
    ...
    ReturnMatrix Helmert(int n, int j, bool full)
    {
        ...
        if (n <= 0)
            Throw(ProgramException("X Vector of length <= 0"));
        if (j > n || j <= 0)
        ...
        Y = 0.0;
        if (j > 1)
            Y(j - 1) = sqrt((Real)(j - 1) / (Real)j);
        for (int i = j; i < n; ++i)
            Y(i) = -1.0 / sqrt((Real)i * (i + 1));
        if (full)
            Y(n) = 1.0 / sqrt((Real)n);
        Y.release();
        return Y.for_return();
    ...
    ReturnMatrix Helmert(const Matrix &X, bool full)
    {
        ...
        int n = X.ncols();
        if (m == 0)
            Throw(ProgramException("Matrix has 0 rows ", X));
        Matrix Y;
        if (full)
            Y.resize(m, n);
        else
