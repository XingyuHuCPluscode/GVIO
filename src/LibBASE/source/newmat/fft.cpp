...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 19); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
    ...
    // minimise roundoff error
    {
        REPORT
        long n4 = n * 4;
        int sector = (int)floor((Real)n4 / (Real)d + 0.5);
        n4 -= sector * d;
        if (sector < 0)
        {
    ...
    void RealFFT(const ColumnVector &U, ColumnVector &X, ColumnVector &Y)
    {
        ...
        i = n2 - 1;
        a = A.Store();
        b = B.Store(); // first els of A and B
        Real *an = a + i;
        Real *bn = b + i; // last els of A and B
        Real *x = X.Store();
        Real *y = Y.Store(); // first els of X and Y
        Real *xn = x + n2;
        Real *yn = y + n2; // last els of X and Y

        *x++ = *a + *b;
        *y++ = 0.0; // first complex element
        *xn-- = *a++ - *b++;
        *yn-- = 0.0; // last complex element
    ...
    void RealFFTI(const ColumnVector &A, const ColumnVector &B, ColumnVector &U)
    {
        ...
        Real *a = A.Store();
        Real *b = B.Store(); // first els of A and B
        Real *an = a + n2;
        Real *bn = b + n2; // last els of A and B
        Real *x = X.Store();
        Real *y = Y.Store(); // first els of X and Y
        Real *xn = x + i;
        Real *yn = y + i; // last els of X and Y
             ...
             ColumnVector &X, ColumnVector &Y)
    {
        ...
        if (!FFT_Controller::OnlyOldFFT && FFT_Controller::CanFactor(n))
        {
            ...
            X = U;
            Y = V;
            if (FFT_Controller::ar_1d_ft(n, X.Store(), Y.Store()))
                return;
        }
        ...
        bool inzee = true;
        int now = 0;
        int b1; // initialised to keep gnu happy
        ...
        do
        {
            ...
            {
                REPORT fftstep(X, Y, A, B, after, now, before);
            }
