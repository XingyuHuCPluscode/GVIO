...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 17); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
...
*/
                      ...
                      DiagonalMatrix &E, SymmetricMatrix &A)
    {
        ...
        {
            Real h = 0.0;
            Real f = -FloatingPointPrecision::Maximum();
            Real *d = D.Store();
            Real *a = A.Store() + (i * (i + 1)) / 2;
            int k = i;
            while (k--)
            {
                f = *a++;
    ...
    static void tql1(DiagonalMatrix &D, DiagonalMatrix &E)
    {
        ...
        for (l = 0; l < n; l++)
        {
            ...
            for (i = l; i > 0; i--)
            {
                if (p < D.element(i - 1))
                {
                    REPORT D.element(i) = D.element(i - 1);
                }
                else
                {
                    REPORT test = true;
