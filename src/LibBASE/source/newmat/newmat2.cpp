...
#define REPORT                                   \
    {                                            \
        static ExeCounter ExeCount(__LINE__, 2); \
        ++ExeCount;                              \
    }
#else
#define REPORT \
    ...
    //#define MONITOR(what,storage,store) { std::cout << what << " " << storage << " at " << (long)store << "\n"; }
...
#define MONITOR(what, store, storage) \
    ...
    void MatrixRowCol::Add(const MatrixRowCol &mrc)
    {
        ...
        if (l <= 0)
            return;
        Real *elx = data + (f - skip);
        Real *el = mrc.data + (f - mrc.skip);
        while (l--)
            *elx++ += *el++;
    }
    ...
    Real DotProd(const MatrixRowCol &mrc1, const MatrixRowCol &mrc2)
    {
        REPORT // not accessed
            int f = mrc1.skip;
        int f2 = mrc2.skip;
        int l = f + mrc1.storage;
        int l2 = f2 + mrc2.storage;
        if (f < f2)
            f = f2;
        if (l > l2)
            l = l2;
        ...
        Real *el1 = mrc1.data + (f - mrc1.skip);
        Real *el2 = mrc2.data + (f - mrc2.skip);
        Real sum = 0.0;
        while (l--)
            sum += *el1++ * *el2++;
        return sum;
    ...
    void MatrixRowCol::Add(const MatrixRowCol &mrc1, const MatrixRowCol &mrc2)
    {
        ...
        Real *el2 = mrc2.data + (f2 - mrc2.skip);
        if (f1 < f2)
        {
            int i = f1 - f;
            while (i--)
    ...
    void MatrixRowCol::Add(const MatrixRowCol &mrc1, Real x)
    {
        ...
        Real *elx = data;
        Real *ely = mrc1.data + (f - mrc1.skip);
    ...
    void MatrixRowCol::ConCat(const MatrixRowCol &mrc1, const MatrixRowCol &mrc2)
    {
        ...
        {
            Real *ely = mrc1.data + (f1 - mrc1.skip);
            while (i--)
                *elx++ = *ely++;
        }
        ...
        i = mrc1.length;
        int skipx = l1 - i;
        lx -= i; // addresses rel to second seg, maybe -ve
        if (f2 < skipx)
        {
            f2 = skipx;
            if (l2 < f2)
        ...
        {
            Real *ely = mrc2.data + (f2 - mrc2.skip);
            while (i--)
                *elx++ = *ely++;
        }
        lx -= l2;
    ...
    // row for Kronecker product
    {
        ...
        i = mrc1.storage;
        Real *el1 = mrc1.data;
        int mrc2_skip = mrc2.skip;
        int mrc2_storage = mrc2.storage;
