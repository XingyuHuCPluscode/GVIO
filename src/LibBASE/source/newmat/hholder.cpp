...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 16); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
...
*/
    ...
    void updateQRZT(Matrix &X, LowerTriangularMatrix &L)
    {
        ...
        for (int i = 0; i < s; i++)
        {
            ...
            {
                Real frs = fabs(r) + sum;
                Real a0 = sqrt(frs / sum);
                Real alpha = a0 / frs;
                if (r <= 0)
                {
                    REPORT L.element(i, i) = sum;
                    alpha = -alpha;
                }
                else
                {
    ...
    void extend_orthonormal(Matrix &A, int n)
    {
        ...
        int nc = A.ncols();
        if (nc > nr)
            Throw(IncompatibleDimensionsException(A));
        if (n > nc)
            Throw(IncompatibleDimensionsException(A));
        ColumnVector SSR;
        {
            Matrix A1 = A.Columns(1, n);
            SSR = A1.sum_square_rows();
        }
        for (int i = n; i < nc; ++i)
        {
