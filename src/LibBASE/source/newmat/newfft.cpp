...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 20); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
                         ...
                         Real *X, Real *Y)
    {
        ...
        for (J = P_SYM; J <= JL; J += P_SYM)
        {
            K = J;
            do
                K = P_SYM * BitReverse(K / P_SYM, P_UN_SYM, N_UN_SYM, UN_SYM);
            while (K < J);
        ...
        L700:
            ...
            for (K = K0; K < N; K += MP)
            {
                ...
                for (U = 1; U <= PP; U++)
                {
                    ...
                    for (V = 0; V < PP; V++)
                    {
                        RA[V] = RA[V] + RS * AA[V][U - 1];
                        IA[V] = IA[V] + IS * AA[V][U - 1];
                        RB[V] = RB[V] + RU * BB[V][U - 1];
                        IB[V] = IB[V] + IU * BB[V][U - 1];
                    }
                }
                X[K] = XT;
                Y[K] = YT;
                for (U = 1; U <= PP; U++)
                {
                    if (!ZERO)
                    {
                        ...
                        XT = RA[U - 1] + IB[U - 1];
                        YT = IA[U - 1] - RB[U - 1];
                        X[K + M * U] = XT * C[U - 1] + YT * S[U - 1];
                        Y[K + M * U] = YT * C[U - 1] - XT * S[U - 1];
                        JJ = P - U;
                        XT = RA[U - 1] - IB[U - 1];
                        YT = IA[U - 1] + RB[U - 1];
                    ...
                    {
                        REPORT
                        X[K + M * U] = RA[U - 1] + IB[U - 1];
                        Y[K + M * U] = IA[U - 1] - RB[U - 1];
                        JJ = P - U;
                        X[K + M * JJ] = RA[U - 1] - IB[U - 1];
                        Y[K + M * JJ] = IA[U - 1] + RB[U - 1];
                    }
                }
            }
            goto L500;
...

    {
        ...
        A1 = cos(TWOPI / 5.0);
        B1 = sin(TWOPI / 5.0);
        A2 = cos(2.0 * TWOPI / 5.0);
        B2 = sin(2.0 * TWOPI / 5.0);

        for (J = 0; J < M_OVER_2; J++)
        {
            NO_FOLD = (J == 0 || 2 * J == M);
            K0 = J;
            ANGLE = TWOPI * Real(J) / Real(M5);
            ZERO = ANGLE == 0.0;
            C1 = cos(ANGLE);
            S1 = sin(ANGLE);
            C2 = C1 * C1 - S1 * S1;
    ...
    //    RADIX SIXTEEN FOURIER TRANSFORM KERNEL
    {
        ...
        EI1 = sin(TWOPI / 16.0);
        E2 = cos(TWOPI / 8.0);
        ER3 = cos(3.0 * TWOPI / 16.0);
        EI3 = sin(3.0 * TWOPI / 16.0);
        ER5 = cos(5.0 * TWOPI / 16.0);
        EI5 = sin(5.0 * TWOPI / 16.0);
        ...
        L200:
            ...
            for (K = K0; K < N; K += M16)
            {
                ...
                RSU3 = T;
                T = RUS1 * ER3 + IUS1 * EI3;
                IUS1 = IUS1 * ER3 - RUS1 * EI3;
                RUS1 = T;
                T = (RUS2 + IUS2) * E2;
                IUS2 = (IUS2 - RUS2) * E2;
                RUS2 = T;
                T = RUS3 * ER5 + IUS3 * EI5;
                IUS3 = IUS3 * ER5 - RUS3 * EI5;
                RUS3 = T;
                T = RUU1 * ER1 + IUU1 * EI1;
                IUU1 = IUU1 * ER1 - RUU1 * EI1;
                RUU1 = T;
                T = (RUU2 + IUU2) * E2;
                IUU2 = (IUU2 - RUU2) * E2;
                RUU2 = T;
                T = RUU3 * ER3 + IUU3 * EI3;
                IUU3 = IUU3 * ER3 - RUU3 * EI3;
                RUU3 = T;
                RSSS0 = RSS0 + RSS2;
                ISSS0 = ISS0 + ISS2;
                RSSS1 = RSS1 + RSS3;
                ...
                if (!ZERO)
                {
                    ...
                    X9[K] = R9 * C9 + I9 * S9;
                    Y9[K] = I9 * C9 - R9 * S9;
                    X5[K] = R10 * C10 + I10 * S10;
                    Y5[K] = I10 * C10 - R10 * S10;
                    X13[K] = R11 * C11 + I11 * S11;
                    Y13[K] = I11 * C11 - R11 * S11;
                    X3[K] = R12 * C12 + I12 * S12;
                    Y3[K] = I12 * C12 - R12 * S12;
                    X11[K] = R13 * C13 + I13 * S13;
                    Y11[K] = I13 * C13 - R13 * S13;
                    X7[K] = R14 * C14 + I14 * S14;
                    Y7[K] = I14 * C14 - R14 * S14;
                    X15[K] = R15 * C15 + I15 * S15;
                    Y15[K] = I15 * C15 - R15 * S15;
                }
                else
                {
                    REPORT
