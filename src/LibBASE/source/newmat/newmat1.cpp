...
#define REPORT                                   \
    {                                            \
        static ExeCounter ExeCount(__LINE__, 1); \
        ++ExeCount;                              \
    }
#else
#define REPORT \
    ...
    {
        REPORT
        int a = ((attribute | mt.attribute) & ~(Symmetric + Skew + Valid + Ones)) | (attribute & mt.attribute);
        if ((a & Lower) != 0 && (a & Upper) != 0)
            a |= Diagonal;
        if ((attribute & Skew) != 0)
        {
            if ((mt.attribute & Symmetric) != 0)
                a |= Skew;
            if ((mt.attribute & Skew) != 0)
            {
                a &= ~Skew;
                a |= Symmetric;
            }
    ...
    {
        REPORT
        return ((a.attribute | b.attribute | c.attribute) & ~(MatrixType::Valid | MatrixType::Square)) == 0;
    }

    const char *MatrixType::value() const
    {
        // make a std::string with the name of matrix with the given attributes
        switch (attribute)
        {
        case Valid:
            REPORT return "Rect ";
    ...
    GeneralMatrix *MatrixType::New(int nr, int nc, BaseMatrix *bm) const
    {
        ...
        Tracer tr("New");
        GeneralMatrix *gm = 0; // initialised to keep gnu happy
        switch (attribute)
        {
        case Valid:
            REPORT
        ...
        case Valid + Square:
            ...
            gm = new SquareMatrix(nr);
            break;

        case Valid + Symmetric + Square:
            REPORT gm = new SymmetricMatrix(nr);
            break;
        ...
        case Valid + Band + Square:
        {
            ...
            gm = new BandMatrix(nr, bw.lower_val, bw.upper_val);
            break;
        }

        case Valid + Symmetric + Band + Square:
            REPORT gm = new SymmetricBandMatrix(nr, bm->bandwidth().lower_val);
            break;

        case Valid + Upper + Square:
            REPORT gm = new UpperTriangularMatrix(nr);
            break;

        case Valid + Diagonal + Symmetric + Band + Upper + Lower + Square:
            REPORT gm = new DiagonalMatrix(nr);
            break;

        case Valid + Band + Upper + Square:
            REPORT gm = new UpperBandMatrix(nr, bm->bandwidth().upper_val);
            break;

        case Valid + Lower + Square:
            REPORT gm = new LowerTriangularMatrix(nr);
            break;

        case Valid + Band + Lower + Square:
            REPORT gm = new LowerBandMatrix(nr, bm->bandwidth().lower_val);
            break;

        case Valid + Diagonal + Symmetric + Band + Upper + Lower + Ones + Square:
            REPORT gm = new IdentityMatrix(nr);
            break;
