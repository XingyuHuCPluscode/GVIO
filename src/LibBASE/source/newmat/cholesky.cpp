...
#define REPORT                                    \
    {                                             \
        static ExeCounter ExeCount(__LINE__, 14); \
        ++ExeCount;                               \
    }
#else
#define REPORT \
    ...
    {
        int nc = chol.Nrows();
        ColumnVector cGivens(nc);
        cGivens = 0.0;
        ColumnVector sGivens(nc);
        sGivens = 0.0;
    ...
    void downdate_Cholesky(UpperTriangularMatrix &chol, RowVector x)
    {
        ...
        // compute and apply Givens rotations to the std::vector a
        ColumnVector cGivens(nRC);
        cGivens = 0.0;
        ColumnVector sGivens(nRC);
        sGivens = 0.0;
        for (i = nRC; i >= 1; i--)
            alpha = pythag(alpha, a(i), cGivens(i), sGivens(i));

        // apply Givens rotations to the jth column of chol
        ColumnVector xtilde(nRC);
        xtilde = 0.0;
        for (j = nRC; j >= 1; j--)
        {
    ...
    void right_circular_update_Cholesky(UpperTriangularMatrix &chol, int k, int l)
    {
        ...
        for (i = 1; i < k; ++i)
            cholCopy(i, k) = columnL(i);

        // II. determine the l-k Given's rotations
        int nGivens = l - k;
        ColumnVector cGivens(nGivens);
        cGivens = 0.0;
        ColumnVector sGivens(nGivens);
        sGivens = 0.0;
        for (i = l; i > k; i--)
        {
