...
namespace RBD_COMMON
{
    ...
    Janitor::Janitor()
    {
        if (do_not_link)
        {
            ...
            OnStack = false;
#ifdef CLEAN_LIST
            std::cout << "Not added to clean-list " << (unsigned long)this << "\n";
#endif
        }
        else
        {
            OnStack = true;
#ifdef CLEAN_LIST
            std::cout << "Add to       clean-list " << (unsigned long)this << "\n";
#endif
            NextJanitor = JumpBase::jl->janitor;
            JumpBase::jl->janitor = this;
        }
    }
    ...
    Janitor::~Janitor()
    {
        ...
        {
#ifdef CLEAN_LIST
            std::cout << "Delete from  clean-list " << (unsigned long)this << "\n";
#endif
            Janitor *lastjan = JumpBase::jl->janitor;
            if (this == lastjan)
                JumpBase::jl->janitor = NextJanitor;
            else
            {
                     ...
                     jan = lastjan->NextJanitor)
                {
                    if (jan == this)
                    {
                        lastjan->NextJanitor = jan->NextJanitor;
                        return;
                    }
                    lastjan = jan;
                }
    ...
    JumpItem *JumpBase::jl; // will be std::set to zero
    jmp_buf JumpBase::env;
    bool Janitor::do_not_link; // will be std::set to false
    ...
    Tracer *Tracer::last; // will be std::set to zero
    ...
    {
        std::cout << "   " << ClassName << "   " << (unsigned long)ClassStore << "\n";
    }

    void FCLRealArray::Report()
    {
        std::cout << "   " << Operation << "   " << (unsigned long)ClassStore << "   " << size << "\n";
    }
    ...
    void FreeCheck::Register(void *t, char *name)
    {
        ...
        if (!f)
        {
            std::cout << "Out of memory in FreeCheck\n";
            exit(1);
        }
#ifdef REG_DEREG
        std::cout << "Registering   " << name << "   " << (unsigned long)t << "\n";
#endif
    }
...
#ifdef REG_DEREG
        std::cout << "Deregistering " << name << "   " << (unsigned long)t << "\n";
#endif
