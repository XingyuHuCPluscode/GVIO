...
#define REPORT                                   \
    {                                            \
        static ExeCounter ExeCount(__LINE__, 4); \
        ++ExeCount;                              \
    }
#else
#define REPORT \
...
#define DO_SEARCH // search for LHS of = in RHS
    ...
    GeneralMatrix::GeneralMatrix(ArrayLengthSpecifier s)
    {
        ...
        if (storage)
        {
            store = new Real[storage];
            MatrixErrorNoSpace(store);
    ...
    CroutMatrix::CroutMatrix(const BaseMatrix &m)
    {
        ...
        {
            REPORT((CroutMatrix *)gm)->get_aux(*this);
            GetMatrix(gm);
        }
        else
        {
            ...
            d = true;
            sing = false;
            indx = new int[nrows_val];
            MatrixErrorNoSpace(indx);
    ...
    void CroutMatrix::get_aux(CroutMatrix &X)
    {
        ...
        else // copy the array
        {
            ...
            int *ix = new int[nrows_val];
            MatrixErrorNoSpace(ix);
    ...
    {
        REPORT GeneralMatrix::resize(nr, nr, 1);
        *store = 1;
    }
...
*/
    ...
    MatrixBandWidth KPMatrix::bandwidth() const
    {
        ...
        else
        {
            ...
            {
                REPORT lower = bw2.Lower() + bw1.Lower() * gm2->Nrows();
            }
        }
        if (bw1.Upper() < 0)
        {
        ...
        else
        {
            ...
            {
                REPORT upper = bw2.Upper() + bw1.Upper() * gm2->Nrows();
            }
        }
    ...
    MatrixBandWidth GetSubMatrix::bandwidth() const
    {
        ...
        {
            REPORT return MatrixBandWidth(-1);
        }
    }
    ...
    void GeneralMatrix::tDelete()
    {
        if (tag_val < 0)
        {
            ...
            {
                REPORT return;
            } // not a temporary matrix - leave alone
        }
        if (tag_val == 1)
        {
            if (store)
            {
                REPORT MONITOR_REAL_DELETE("Free   (tDelete)", storage, store) delete[] store;
            }
            MiniCleanUp();
            return; // CleanUp
        }
    ...
    bool GeneralMatrix::reuse()
    {
        if (tag_val < -1) // borrowed storage
        {
            ...
            {
                REPORT
                Real *s = new Real[storage];
                MatrixErrorNoSpace(s);
    ...
    // If mt == 0 just let *gmx point to storage of *this if tag_val==-1.
    {
        if (!mt)
        {
            if (tag_val == -1)
            {
                REPORT gmx->tag_val = -2;
                gmx->store = store;
            }
            else
            {
                REPORT gmx->tag_val = 0;
                gmx->store = GetStore();
            }
        }
    ...
    // evaluate X and copy to this
    {
        ...
        {
            REPORT Release(counter);
        }
        GeneralMatrix *gmx = ((BaseMatrix &)X).Evaluate(mt);
        if (gmx != this)
        {
            REPORT GetMatrix(gmx);
        }
        else
        {
            REPORT
    ...
    // we know tag_val has been std::set correctly in each.
    {
        GeneralMatrix *gmx = ((BaseMatrix &)X).Evaluate(mt);
        if (gmx != this)
        {
    ...
    {
        REPORT
        GeneralMatrix *gm = new Matrix(*this);
    ...
    {
        REPORT
        GeneralMatrix *gm = new SquareMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new SymmetricMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new DiagonalMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new RowVector(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new ColumnVector(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new nricMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    {
        REPORT
        GeneralMatrix *gm = new IdentityMatrix(*this);
        MatrixErrorNoSpace(gm);
        return gm;
    }
    ...
    void nricMatrix::MakeRowPointer()
    {
        ...
        if (nrows_val > 0)
        {
            row_pointer = new Real *[nrows_val];
            MatrixErrorNoSpace(row_pointer);
            Real *s = Store() - 1;
            int i = nrows_val;
            Real **rp = row_pointer;
            if (i)
    ...
    void GeneralMatrix::CheckStore() const
    {
        REPORT
        if (!store)
            Throw(ProgramException("NRIC accessing matrix with unstd::set dimensions"));
    }
    ...
    {
        // std::set matrix dimensions to zero, delete storage
        REPORT
        if (store && storage)
        {
    ...
    {
        if (n < 0)
            Throw(Logic_error("invalid array length"));
        else if (n == 0)
        {
            REPORT a = 0;
        }
    ...
    // std::set all the elements equal to a given value
    ...
    // std::set the elements equal to those of another array.
    ...
    void GeneralMatrix::swap(GeneralMatrix &gm)
    {
        ...
        gm.tag_val = t;
        t = nrows_val;
        nrows_val = gm.nrows_val;
        gm.nrows_val = t;
        t = ncols_val;
        ncols_val = gm.ncols_val;
        gm.ncols_val = t;
        t = storage;
        storage = gm.storage;
        gm.storage = t;
    ...
    void BandMatrix::swap(BandMatrix &gm)
    {
        ...
        int i;
        i = lower_val;
        lower_val = gm.lower_val;
        gm.lower_val = i;
        i = upper_val;
        upper_val = gm.upper_val;
        gm.upper_val = i;
    }
...
/// Can we add two matrices with simple std::vector add.
/// SimpleAddOK shows when we can add two matrices by a simple std::vector add
