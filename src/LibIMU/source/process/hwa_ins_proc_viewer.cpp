...
hwa_ins::ins_viewer::ins_viewer()
{
    t = nullptr;
    ins_viewer::vptr.push_back(this);
...
hwa_ins::ins_viewer::~ins_viewer()
{
    ins_viewer::vptr.erase(std::find(ins_viewer::vptr.begin(), ins_viewer::vptr.end() - 1, this));
...
void hwa_ins::ins_viewer::Run()
{
    ...
    window = glfwCreateWindow(200, 200, "Viewer", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return;
   ...
   /* Loop until the user closes the window */
    while (!glfwWindowShouldClose(window) && !mb_stop)
    {
        ...
        if (mb_axis)
        {
            ...
            MyCylinder(0.025f * 1 / m_scale, 3, 8);
            glPopMatrix();
            glPushMatrix();
            glRotatef(-90, 1, 0, 0);
    ...
    /*                glColor3f(c_l[0] + (c_h[0] - c_l[0])*h,
        ...
        if (mb_PPs)
        {
            ...
            for (int i = 0; i < near_points.size(); i++)
            {
                ...
                glBegin(GL_POINTS);
                glVertex3f(pcs.at(i)(0), pcs.at(i)(1), pcs.at(i)(2));
                glEnd();
                const auto &pc = near_points.at(i);
                glPointSize(1.0f);
                for (const auto &p : pc)
                {
                    glColor3f(float(r / 255.0), float(g / 255.0), float(b / 255.0));
...
void hwa_ins::ins_viewer::ClearView()
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_trajectory.clear();
    mv_pointCloud.clear();
...
void hwa_ins::ins_viewer::SetFrames(const Frames &f)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_frames.clear();
...
void hwa_ins::ins_viewer::SetPointCloud(const VPointCloud &pc)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_pointCloud.clear();
...
void hwa_ins::ins_viewer::SetFrames(const std::vector<Frames> &vf)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_frames = vf;
}

void hwa_ins::ins_viewer::AddNewFrame(const std::pair<SO3, Triple>& f)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_frames.back().push_back(f);
}

void hwa_ins::ins_viewer::SetPointCloud(const std::vector<VPointCloud> &vpc)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_pointCloud = vpc;
}

void hwa_ins::ins_viewer::SetPlaneCloud(const std::vector<Triple> &_pcs, const std::vector<std::vector<Triple>> & _near_points)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
void hwa_ins::ins_viewer::AddNewPoint(const Triple & p)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_pointCloud.back().push_back(p);
}

void hwa_ins::ins_viewer::AddNewPoint(const std::vector<Triple> &pts)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
void hwa_ins::ins_viewer::SetTrajectory(const std::vector<Trajectory>& vt)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_trajectory = vt;
}

void hwa_ins::ins_viewer::AddNewPos(const Triple & p)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    mv_trajectory.back().push_back(p);
}

void hwa_ins::ins_viewer::Show()
{
    if(t) return;
    mb_stop = false;
