...
public:
	/// 构造
	CAutoLock(CLock objLock):m_objLock(objLock){ m_objLock.Lock(); }
	/// 析构
	virtual ~CAutoLock() { m_objLock.Unlock(); }

private:
	CLock m_objLock; ///< 临界段变量	
};
...
private:
	...
	/// 判断是否为Bayer格式
	bool __IsBayer(GX_PIXEL_FORMAT_ENTRY nPixelFormat) const;
...
private:
	...
	std::map<std::string, int64_t> m_mapPixelFormat;                ///< 字符串到整形的像素格式映射
private:
