...
namespace hwa_gnss
{
    ...
    gnss_amb_bdeci::gnss_amb_bdeci()
    {
    }

    gnss_amb_bdeci::gnss_amb_bdeci(const double &cutdev, const double &cutsig, const double &cutprob, const double &cutdeci)
    {
        this->_cutdev = cutdev;
        this->_cutsig = cutsig;
    ...
    bool gnss_amb_bdeci::bdeci(double dEst, double dSigma, int iIh)
    {
        double prob = 0.0;
        double deci = 0.0;
    ...
    //    !Output :
    //    !prob : area of decision - function region, std::set = 1.0 for
    ...
    //        !a time.)
    void gnss_amb_bdeci::bdeci(double dEst, double dSigma, int iIh, double dCutdev, double dCutsig, double &dProb, double &dDeci)
    {
        ...
        try
        {
            ...
            for (j = 0; j < 50; j++)
            {
                ...
                {
                    dErfcb1 = gnss_amb_bdeci::_erfc(dB1);
                }
                if (dB2 < 0 || dB2 > 15.0)
                {
                    dErfcb2 = 0.0;
                ...
                {
                    dErfcb2 = gnss_amb_bdeci::_erfc(dB2);
                }
        ...
        {
            std::cout << "ERROR : gnss_amb_bdeci::bdeci throw exception" << std::endl;
        }
    }

    void gnss_amb_bdeci::_gser(double &dGamser, double dA, double dX, double &dGln)
    {
        ...
        {
            std::cout << "ERROR : gnss_amb_bdeci::_gser throw exception" << std::endl;
        }
    }

    double gnss_amb_bdeci::_gammln(double dXx)
    {
        ...
        {
            std::cout << "ERROR : gnss_amb_bdeci::_gammln throw exception" << std::endl;
        }
    }

    void gnss_amb_bdeci::_gcf(double &dGammcf, double dA, double dX, double &dGln)
    {
        ...
        const int itmax = 100;
        const double dEps = 3.0e-7;
        dGln = gnss_amb_bdeci::_gammln(dA);
        ...
        {
            std::cout << "ERROR : gnss_amb_bdeci::_gcf throw exception" << std::endl;
        }
    }

    double gnss_amb_bdeci::_gammq(double dA, double dX)
    {
        ...
        try
        {
            ...
            if (dX < (dA + 1))
            {
                gnss_amb_bdeci::_gser(gamser, dA, dX, gln);
                gammq = 1.0 - gamser;
            }
            else
            {
                gnss_amb_bdeci::_gcf(gammq, dA, dX, gln);
            }
            return gammq;
        }
        catch (...)
        {
            std::cout << "ERROR : gnss_amb_bdeci::_gammq throw exception" << std::endl;
            return 0.0;
        }
    }

    double gnss_amb_bdeci::_gammp(double dA, double dX)
    {
        ...
        try
        {
            ...
            if (dX < (dA + 1))
            {
                gnss_amb_bdeci::_gser(gammp, dA, dX, gln);
            }
            else
            {
                gnss_amb_bdeci::_gcf(gammcf, dA, dX, gln);
                gammp = 1.0 - gammcf;
            }
            return gammp;
        }
        ...
        {
            std::cout << "ERROR : gnss_amb_bdeci::_gammp throw exception" << std::endl;
            return 0.0;
        }
    }

    double gnss_amb_bdeci::_erf(double dX)
    {
        double erf, half;
        half = 0.5;
        try
        {
            if (dX < 0)
            {
                erf = -gnss_amb_bdeci::_gammp(half, dX * dX);
            }
            else
            {
                erf = gnss_amb_bdeci::_gammp(half, dX * dX);
        ...
        {
            std::cout << "ERROR : gnss_amb_bdeci::_erf throw exception" << std::endl;
            return 0.0;
        }
    }

    double gnss_amb_bdeci::_erfc(double dX)
    {
        ...
        if (dX < 0)
        {
            erfc = 1.0 + gnss_amb_bdeci::_gammp(half, dX * dX);
        }
        else
        {
            erfc = gnss_amb_bdeci::_gammq(half, dX * dX);
        }
        return erfc;
    }
}