...
namespace hwa_gnss
{
    void gnss_amb_lambda::BACKTs(int iN, int *piI, double *pdEnd, double *pdDist, double *pdLef, double *pdLeft, bool *pdEnde)
    {
        int iJ;
        iJ = *piI + 1;
    ...
    void gnss_amb_lambda::ZTRANi(int iFirst, int iLast, int iN, double *pdL, double *pdA, double *pdZti)
    {
        int i, j, k, iMu;
    ...
    void gnss_amb_lambda::STOREs(int iCan, int iPos, int *piMax, double dT, double *pdTmax, double *pdDist, double *pdDisall, double *pdCands, int iMaxCan, int iN)
    {
        int i;
        for (i = 1; i <= iN; i++)
        {
    ...
    void gnss_amb_lambda::COLLECTs(int iN, int iMaxCan, double dD_1, double dLef_1, double dLeft_1, double dRight_1, double dChic, double *pdDist,
    ...
    */
    int gnss_amb_lambda::FMFAC6(double *pdL, double *pdD, int iN, double dEps)
    {
        double dGomi = 1e20;
        int i, j, k;
        double dSum;
    ...
    void gnss_amb_lambda::INVLT2d(int iN, double *pdL, double *pdLm, double *pdVec)
    {
        int i, j, k;
        double dAiude;
        for (i = 1; i <= iN; i++)
    ...
    void gnss_amb_lambda::SRC1i(int iN, double *pdL, double *pdD, double *pdA, double *pdZti)
    {
        double dgnss_amb_lambda[3];
        bool isSwap;
        int i1, i, j;
            ...
            if (isSwap)
            {
                ...
                while (true)
                {
                    if (!isSwap && i > 1)
                    {
                        ...
                        if (dDelta < pdD[i])
                        {
                            dgnss_amb_lambda[2] = pdD[i] * pdL[i * iN + i - 1] / dDelta;
                            dEta = pdD[i - 1] / dDelta;
                            pdD[i - 1] = dEta * pdD[i];
                            pdD[i] = dDelta;
                            for (j = 1; j <= i - 1; j++)
                            {
                                dgnss_amb_lambda[0] = pdL[(i - 1) * iN + j - 1];
                                dgnss_amb_lambda[1] = pdL[i * iN + j - 1];
                                pdL[(i - 1) * iN + j - 1] = dgnss_amb_lambda[1] - pdL[i * iN + i - 1] * dgnss_amb_lambda[0];
                                pdL[i * iN + j - 1] = dgnss_amb_lambda[2] * dgnss_amb_lambda[1] + dEta * dgnss_amb_lambda[0];
                            }

                            pdL[i * iN + i - 1] = dgnss_amb_lambda[2];
    ...
    void gnss_amb_lambda::CHIstrt4(int iN, double *pdD, double *pdL, double *pdDist, double *pdE, double *pdA, double dTm[2], double *pdT)
    {
        // double dNabla;
        double dBig = 1e10;
        int i, j;
    ...
    double gnss_amb_lambda::DINKi(int iN, double *pdL, double *pdA, int iCol)
    {
        double dDINKi;
        int i;
        dDINKi = 0.0;
    ...
    int gnss_amb_lambda::JNT2(double dD)
    {
        int iJNT2;
        if (dD <= 0.0)
        {
    ...
    void gnss_amb_lambda::FI71(double dChic, int iMaxCan, int iN, double *pdA, double *pdD, double *pdL, double *pdLef, double *pdLeft, double *pdRight,
                         double *pdDist, double *pdEnd, double *pdDq, int *piNcan, double *pdDisall, double *pdCands, int *piPos)
    {
        ...
        {
            //t_glog comment
            //LOG4CPLUS_WARN(Log::getInstance()->GetLogger(), "ERROR in FI71: number of requested candidates < 1");
            throw("ERROR in FI71: number of requested candidates < 1");
        }
        else if (iN < 2)
        {
            //LOG4CPLUS_WARN(Log::getInstance()->GetLogger(), "ERROR in FI71: dimension of system < 2");
        ...
        *piNcan = 0;
        ...
        while (true)
        {
            ...
            {
                pdLef[i - 1] = gnss_amb_lambda::DINKi(iN, pdL, pdDist, i);
            }
            iold = i;
            ...
            pdDist[i - 1] = gnss_amb_lambda::JNT2(dDelta) - pdA[i - 1];
            if (pdDist[i - 1] > (dReach - pdLef[i - 1]))
            {
                gnss_amb_lambda::BACKTs(iN, &i, pdEnd, pdDist, pdLef, pdLeft, &isEnde);
            }
            else
            {
            ...
            if (i == 1)
            {
                gnss_amb_lambda::COLLECTs(iN, iMaxCan, pdD[0], pdLef[0], pdLeft[0], pdRight[0],
    ...
    void gnss_amb_lambda::LAMBDA4(int iMaxCan, int iN, double *pdQ, double *pdA, int *piNcan, int *piPos, double *pdCands, double *pdDisall, double *boot)
    {
        ...
        for (int j = 0; j < iN; j++){
            for (int k = 0; k < iN;k++) std::cout <<" "<< pdL[j * iN + k] ;
            std::cout << std::endl;
        }
        if (fabs(pdL[0]-0.0997835)<1e-6) {
            int testlhb = 1;
        }*/
        if (gnss_amb_lambda::FMFAC6(pdL, pdD, iN, dEps) < 0)
        {
            delete[] pdL;
            pdL = nullptr;
            delete[] pdD;
        ...
        //std::cout << "pdD" << std::endl;
        //for (int j = 0; j < iN; j++) std::cout << pdD[j] << std::endl;
        /*compute the Z - transformation based on L and D of Q,
        ambiguities are transformed according to \hat{ z } = Z^* \hat{ a }*/
        gnss_amb_lambda::SRC1i(iN, pdL, pdD, pdA, pdZt);
        ...
        *boot = 1.0;
        ...
        the gnss_amb_lambda - transformed L and D as they came from SRC1)*/
        gnss_amb_lambda::INVLT2d(iN, pdL, pdL, pdV1);
        ...
        dEps = 1e-6;
        gnss_amb_lambda::CHIstrt4(iN, pdD, pdL, pdV1, pdV2, pdA, pdV3, pdV4);
        dChi_1 = pdV3[1] + dEps;

        /*find the two candidates with minimum norm*/
        gnss_amb_lambda::FI71(dChi_1, iMaxCan, iN, pdA, pdD, pdL, pdV1, pdV2, pdV3, pdV4,
        ...
        /*'sort' the std::vector of squared norms in increasing order
    ...
    double gnss_amb_lambda::pBootStrapping(const double &sig)
    {
        ...
        else
        {
            ...
            //printf("sig=%-7.4f %7.5f  %3d-%3d  %7.5f %7.5f\n", sig, dRes, i, j, BootSt_2Phi_sigRecip_1[i], BootSt_2Phi_sigRecip_1[j]);
            //getchar();
        }
