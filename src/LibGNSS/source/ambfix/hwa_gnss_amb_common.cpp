...
namespace hwa_gnss
{
    ...
    gnss_amb_epoch_res::gnss_amb_epoch_res()
    {
        wl_res = 0.0;
        wl_fixed = false;
        nl_res = 0.0;
    ...
    void getMeanFract(std::list<std::pair<double, double>> &values, double &mean, double &sigma, double &sigx)
    {
        try
        {
            ...
            double tempfract;
            double mean2, sigma2;
            std::list<std::pair<double, double>> fracts[3];

            //fraction range is (0,1)
            for (auto it_val = values.begin(); it_val != values.end(); it_val++)
            {
                tempfract = it_val->first - round(it_val->first);
                if (tempfract < 0.0)
                    tempfract += 1.0;
                fracts[0].push_back(std::make_pair(tempfract, it_val->second));
            }
            ...
            for (auto it_val = values.begin(); it_val != values.end(); it_val++)
            {
                tempfract = getFraction(it_val->first, -0.50);
                fracts[1].push_back(std::make_pair(tempfract, it_val->second));
            }

            getMeanWgt(true, fracts[1], mean2, sigma2, sigx);

            if (sigma2 < sigma && !hwa_base::double_eq(sigma, sigma2)) // to be same with fortran add "="
                mean = mean2;
            // to avoid bias
            ...
            for (auto it_val = values.begin(); it_val != values.end(); it_val++)
            {
                tempfract = it_val->first - mean;
                tempfract -= round(tempfract);
                fracts[2].push_back(std::make_pair(tempfract, it_val->second));
            }
        ...
        {
            std::cout << "ERROR: t_gmath::get_meanfract , unknown mistake";
            throw(-1);
        }
    }
    ...
    void getMeanWgt(bool is_edit, std::list<std::pair<double, double>> &wx, double &mean, double &sigma, double &mean_sig)
    {
        ...
        {
            std::cout << "ERROR: t_gmath::get_wgt_mean , unknown mistake";
            throw(-1);
        }
    }
    ...
    double getFraction(double x, double min)
    {
        ...
        else if (hwa_base::double_eq(min, -0.50))
        {
            if (fract < -0.5)
            {
                fract += 1.0;
    ...
    gnss_amb_oneway::gnss_amb_oneway()
    {
    }
