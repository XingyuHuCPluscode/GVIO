...
namespace hwa_gnss
{
    gnss_data_navgps::gnss_data_navgps()
        : gnss_data_nav(),
          _iode(0),
          _iodc(0),
          _health(0),
          ...
          _flag(0.0)
    {
        ...
        id_type(base_data::EPHGPS);
        id_group(base_data::GRP_EPHEM);
    }

    // desctructor
    // ----------
    gnss_data_navgps::gnss_data_navgps(base_log spdlog)
    ...
    // print function
    // ----------
    std::string gnss_data_navgps::line() const
    {
    ...
    // print function
    // ----------
    std::string gnss_data_navgps::linefmt() const
    {
    ...
    // ----------
    int gnss_data_navgps::chk(std::set<std::string> &msg)
    {
        if (!_healthy())
        {
        ...
        if ((_tot - _toc) > 86400 || (_toc - _tot) > gnss_data_nav::nav_validity(GPS))
        {
            msg.insert("Issue: " + _sat + " nav large difference [tot-toc] " + base_type_conv::dbl2str(_tot - _toc) + " s " + _tot.str_ymdhms() + _toc.str_ymdhms(" "));
            _validity = false;
        }

        if ((_tot - _toe) > 86400 || (_toe - _tot) > gnss_data_nav::nav_validity(GPS))
        {
    ...
    // ----------
    int gnss_data_navgps::pos(const base_time &t, double xyz[], double var[], double vel[], bool chk_health)
    {
    ...
    // ----------
    int gnss_data_navgps::clk(const base_time &t, double *clk, double *var, double *dclk, bool chk_health)
    {
    ...
    // ----------
    int gnss_data_navgps::data2nav(std::string sat, const base_time &ep, const gnss_data_navDATA &data)
    {
        char tmp[12];
        ...
        if (data_27 == 0.9999e9 || data_27 == 0)
            ...
            _tot = _toe; //20200321 lvhb if tot sow is empty,but there for novatel nav
        else
        {
    ...
    // ----------
    int gnss_data_navgps::nav2data(gnss_data_navDATA &data)
    {
        if (!this->_valid())
            return -1;
    ...
    bool gnss_data_navgps::chktot(const base_time &t)
    {
        if (t > _tot)
            return true;
        else
    ...
    // get parameter value
    // ----------
    hwa_map_time_value gnss_data_navgps::param(const NAVDATA &n)
    {
    ...
    // ----------
    int gnss_data_navgps::param(const NAVDATA &n, double val)
    {
        switch (n)
    ...
    // healthy check
    // ----------
    bool gnss_data_navgps::_healthy() const
    {
        if (_health == 0)
            return true;
        return false;
    ...
    double gnss_data_navgps::_ura_ed() const
    {
        if (_acc == 1)
            return 2.8;
        else if (_acc == 3)
    ...
    // mean_motion
    // ----------
    double gnss_data_navgps::_mean_motion()
    {
    ...
    // eccentric anomaly
    // ----------
    void gnss_data_navgps::_ecc_anomaly(double dt, double &Ek, double &dEk)
    {
