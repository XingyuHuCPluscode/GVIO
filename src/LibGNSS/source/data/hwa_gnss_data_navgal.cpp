...
namespace hwa_gnss
{
    gnss_data_navgal::gnss_data_navgal()
        : gnss_data_nav(),
          _iode(0),
          _health(0),
          _toe(base_time::GAL),
    ...
    {
        _tgd[0] = _tgd[1] = 0.0;
        id_type(base_data::EPHGAL);
        id_group(base_data::GRP_EPHEM);
    }
    gnss_data_navgal::gnss_data_navgal(base_log spdlog)
    ...
    // ----------
    /*
int gnss_data_navgal::operator<(const gnss_data_navgal& data) const
{       
...
*/
    ...
    // ----------
    /*
int gnss_data_navgal_struct::operator==(const gnss_data_navgal& data) const
{       
  int _dtot = int( tot.diff( data.tot ) );
...
*/
    ...
    // print function
    // ----------
    std::string gnss_data_navgal::line() const
    {
        int w = 20;
        ...
        tmp << " " << std::setw(3) << sat()
            ...
            << std::setw(w) << _cis
            << std::setw(w) << _cus
            << std::setw(w) << _tgd[0] // E5a/E1
            << std::setw(w) << _tgd[1] // E5b/E1
            << std::endl;
    ...
    // print function
    // ----------
    std::string gnss_data_navgal::linefmt() const
    {

        std::ostringstream tmp;

        std::string source_bin = _source_str();
        std::string health_bin = _health_str();
        ...
        tmp << " " << std::setw(3) << sat() << std::fixed
            ...
            << " |"
            << std::setw(9) << std::setprecision(3) << _tgd[0] * 1e9 // E5a/E1
            << std::setw(9) << std::setprecision(3) << _tgd[1] * 1e9 // E5b/E1
            << " |"
            << std::setw(11) << health_bin
            << std::setw(11) << source_bin
            << std::setw(4) << src(true)
            << std::setw(9) << gnavtype2str(_gnavtype(true));
    ...
    // ----------
    int gnss_data_navgal::chk(std::set<std::string> &msg)
    {
        std::string binary = _source_str();
        ...
        if ((_tot - _toc) > 86400 || (_toc - _tot) > gnss_data_nav::nav_validity(GAL))
        {
            msg.insert("Issue: " + _sat + " nav large difference [tot-toc] " + base_type_conv::dbl2str(_tot - _toc) + " s " + _tot.str_ymdhms() + _toc.str_ymdhms(" "));
            _validity = false;
        }

        if ((_tot - _toe) > 86400 || (_toe - _tot) > gnss_data_nav::nav_validity(GAL))
        {
        ...
        {
            msg.insert("Issue: " + _sat + " nav invalid [iode] " + _toe.str_ymdhms() + " " + base_type_conv::int2str(_iode));
            _validity = false;
        }

        if (_a < 20000000.0)
        {
            msg.insert("Issue: " + _sat + " nav invalid [a] " + _toe.str_ymdhms() + " " + base_type_conv::dbl2str(_a));
            _validity = false;
        }
        return 0;
    }
    ...
    // return URA
    // ----------
    /* int gnss_data_navgal::ura( double acc ) const */
    /* { */
    /*    
  /*   unsigned int i; */
    ...
    // ----------
    int gnss_data_navgal::pos(const base_time &t, double xyz[], double var[], double vel[], bool chk_health)
    {
    ...
    // ----------
    int gnss_data_navgal::clk(const base_time &t, double *clk, double *var, double *dclk, bool chk_health)
    {
    ...
    int gnss_data_navgal::data2nav(std::string sat, const base_time &ep, const gnss_data_navDATA &data)
    {
        char tmp[12];
    ...
    // ----------
    int gnss_data_navgal::nav2data(gnss_data_navDATA &data)
    {
        if (!this->_valid())
            return -1;
    ...
    bool gnss_data_navgal::chktot(const base_time &t)
    {
        if (t > _tot)
            return true;
        else
    ...
    // get parameter value
    // ----------
    hwa_map_time_value gnss_data_navgal::param(const NAVDATA &n)
    {
    ...
    // ----------
    int gnss_data_navgal::param(const NAVDATA &n, double val)
    {
        switch (n)
    ...
    // source value
    // ----------
    int gnss_data_navgal::src(bool full) const
    {
    ...
    // ----------
    GNAVTYPE gnss_data_navgal::gnavtype(bool full) const
    {
    ...
    // return type of navig. mess.
    // ----------
    GNAVTYPE gnss_data_navgal::_gnavtype(bool full) const
    {
    ...
    // source std::string
    // ----------
    std::string gnss_data_navgal::_source_str() const
    {

        std::string source_bin = std::bitset<10>(_source).to_string();
    ...
    // healthy std::string
    // ----------
    std::string gnss_data_navgal::_health_str() const
    {

        std::string health_bin = std::bitset<9>(_health).to_string();
    ...
    // healthy check
    // ----------
    bool gnss_data_navgal::_healthy() const
    {
...
#ifdef DEBUG
        //  if (_sat == "E14" )
        std::cerr << _toe.str_ymdhms() << " " << _sat << " "
             << "Source: " << source_bin << " "
             << "Health: " << health_bin << " " << gnavtype2str(_gnavtype()); // << std::endl;
#endif
            ...
            source_bin.compare(2, 1, "1") == 0)
        {
            ...
            //          if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E01-B + E4b-I healthy \n";
            //    else{ if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E01-B + E4b-I unhealthy \n"; return false; }
        }
        ...
        if (source_bin.compare(0, 1, "1") == 0)
        {
            ...
            //          if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E01-B healthy \n"; return true; } // HS && DVS
            //    else{ if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E01-B unhealthy \n";  return false; }
        }
        ...
        if (source_bin.compare(1, 1, "1") == 0)
        {
            ...
            //          if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E5a-I healthy \n"; return true; } // HS && DVS
            //    else{ if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E5a-I unhealthy \n";  return false; }
        }
        ...
        if (source_bin.compare(2, 1, "1") == 0)
        {
            ...
            //          if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E5b-I healthy \n"; return true; } // HS && DVS
            //    else{ if( _sat == "E14") std::cerr << std::setw(9) << gnavtype2str(_gnavtype(true)) << " INAV E5b-I unhealthy \n";  return false; }
        }

#ifdef DEBUG
        //  if (_sat == "E14" )
        std::cerr << "Warning: unknown source ! "
             << _toe.str_ymdhms() << " " << _sat << " "
             << "Source: " << source_bin << " " << _source << " "
             << "Health: " << health_bin << " " << _health << " " << gnavtype2str(gnavtype()) << std::endl;
#endif
    ...
    // mean motion
    // ----------
    double gnss_data_navgal::_mean_motion()
    {
    ...
    // eccentric anomaly
    // ----------
    void gnss_data_navgal::_ecc_anomaly(double dt, double &Ek, double &dEk)
    {
