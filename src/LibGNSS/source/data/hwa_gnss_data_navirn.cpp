...
namespace hwa_gnss
{
    gnss_data_navirn::gnss_data_navirn()
        : gnss_data_nav(),
          _iode(0),
          _iodc(0),
          _health(0),
          ...
          _acc(0.0)
    {
        ...
        id_type(base_data::EPHIRN);
        id_group(base_data::GRP_EPHEM);
    }
    gnss_data_navirn::gnss_data_navirn(base_log spdlog)
    ...
    // ----------
    /*
int gnss_data_navirn::operator<(const gnss_data_navirn& data) const
{       
...
*/
    ...
    // ----------
    /*
int gnss_data_navirn_struct::operator==(const gnss_data_navirn& data) const
{       
  int _dtot = int( tot.diff( data.tot ) );
...
*/
    ...
    // print function
    // ----------
    std::string gnss_data_navirn::line() const
    {
    ...
    // print function
    // ----------
    std::string gnss_data_navirn::linefmt() const
    {
    ...
    // ----------
    int gnss_data_navirn::chk(std::set<std::string> &msg)
    {
        if (!_healthy())
        {
        ...
        if ((_tot - _toc) > 86400 || (_toc - _tot) > gnss_data_nav::nav_validity(IRN))
        {
            msg.insert("Issue: " + _sat + " nav large difference [tot-toc] " + base_type_conv::dbl2str(_tot - _toc) + " s " + _tot.str_ymdhms() + _toc.str_ymdhms(" "));
            _validity = false;
        }

        if ((_tot - _toe) > 86400 || (_toe - _tot) > gnss_data_nav::nav_validity(IRN))
        {
    ...
    // return URA
    // ----------
    /* int gnss_data_navirn::ura( double acc ) const */
    /* { */
    /*    
  /*   unsigned int i; */
    ...
    // ----------
    int gnss_data_navirn::pos(const base_time &t, double xyz[], double var[], double vel[], bool chk_health)
    {
    ...
    // ----------
    int gnss_data_navirn::clk(const base_time &t, double *clk, double *var, double *dclk, bool chk_health)
    {
    ...
    // convert general gnavdata structure to gnavirn_struct
    //
    //   ToC, ToE, ToT not definitely implemented!
    // ----------
    int gnss_data_navirn::data2nav(std::string sat, const base_time &ep, const gnss_data_navDATA &data)
    {
        char tmp[12];
    ...
    // ----------
    int gnss_data_navirn::nav2data(gnss_data_navDATA &data)
    {
        if (!this->_valid())
            return -1;
    ...
    bool gnss_data_navirn::chktot(const base_time &t)
    {
        if (t > _tot)
            return true;
        else
    ...
    // get parameter value
    // ----------
    hwa_map_time_value gnss_data_navirn::param(const NAVDATA &n)
    {
    ...
    // ----------
    int gnss_data_navirn::param(const NAVDATA &n, double val)
    {
        switch (n)
    ...
    // healthy check
    // ----------
    bool gnss_data_navirn::_healthy() const
    {
        if (_health == 0)
            return true;
        return false;
    ...
    // mean motion
    // ----------
    double gnss_data_navirn::_mean_motion()
    {
    ...
    // eccentric anomaly
    // ----------
    void gnss_data_navirn::_ecc_anomaly(double dt, double &Ek, double &dEk)
    {
