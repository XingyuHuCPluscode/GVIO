...
namespace hwa_gnss
{
    gnss_coder_dvpteph405::gnss_coder_dvpteph405(set_base *s, string version, int sz)
    ...
    int gnss_coder_dvpteph405::decode_head(char *buff, int bufLen, vector<string> &errmsg)
    {
        _mutex.lock();

        if (base_coder::_add2buffer(buff, bufLen) == 0)
        {
            _mutex.unlock();
            return 0;
        }
    ...
    int gnss_coder_dvpteph405::decode_data(char *buff, int bufLen, int &cnt, vector<string> &errmsg)
    {
        ...
        try
        {
            ...
            while (tmp.length() >= 4 * irecsz)
            {
                ...
                if (_flag == 0)
                {
                    ...
                    while (it != _data.end())
                    {
                        if (it->second->id_type() == base_data::ALLDE)
                        {
                            dynamic_cast<gnss_data_navde *>(it->second)->add_head(SS, au, emrat, _ipt, constname, constval);
                        }
                        it++;
                    }
                    base_coder::_consume(2 * 4 * irecsz);
                    _flag++;
                }
                ...
                else if (_flag > 0)
                {
                    ...
                    while (it != _data.end())
                    {
                        ...
                        if (it->second->id_type() == base_data::ALLDE)
                        {
                            dynamic_cast<gnss_data_navde *>(it->second)->add_data(_flag - 1, coeff);
                        }
                        it++;
                    }
                    _flag++;
                    base_coder::_consume(4 * irecsz);
                }
        ...
        {
            if (_spdlog)
                SPDLOG_LOGGER_ERROR(_spdlog, "ERROR : gnss_coder_dvpteph405::decode_data throw exception");
            return -1;
        }
    }
