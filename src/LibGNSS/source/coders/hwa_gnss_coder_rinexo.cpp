...
namespace hwa_gnss
{
    ...
    // constructor
    // ----------
    gnss_coder_rinexo::gnss_coder_rinexo(set_base *s, string version, int sz)
        : base_coder(s, version, sz), gnss_coder_rinexo3(s, version, sz)
    {
    }
    gnss_coder_rinexo::gnss_coder_rinexo(base_time beg, base_time end, set_base *s, string version, int sz)
        : base_coder(beg, end, s, version, sz), gnss_coder_rinexo3(beg, end, s, version, sz)
    {
    }
    ...
    // ----------
    int gnss_coder_rinexo::decode_head(char *buff, int sz, vector<string> &errmsg)
    {

        _mutex.lock();

        int add = gnss_base_coder::_add2buffer(buff, sz);
        if (add < 0)
        {
            _mutex.unlock();
            return -1;
    ...
    // ----------
    int gnss_coder_rinexo::decode_data(char *buff, int sz, int &cnt, vector<string> &errmsg)
    {
    ...
    // RINEX encoder
    // ---------
    int gnss_coder_rinexo::encode_head(char *buff, int sz, vector<string> &errmsg)
    {
        ...
        if (_ss_position == 0)
        {
            ...
            for (it = _data.begin(); it != _data.end(); ++it)
            {
                if (it->second->id_type() == base_data::ALLOBJ)
                {
                    ...
                    map<string, shared_ptr<gnss_data_obj>> allGrec = all_obj->objects(base_data::REC);
                    if (allGrec.find(_encode_site) != allGrec.end())
                    {
                        shared_ptr<gnss_data_rec> grecCasted = std::dynamic_pointer_cast<gnss_data_rec>(allGrec[_encode_site]);
                        gnss_data_rec::hwa_map_hdr hdrFromGrec = grecCasted->gethdr();
                        if (hdrFromGrec.begin() != hdrFromGrec.end())
                        {
                            gnss_data_rnxhdr rnxHdr = hdrFromGrec.begin()->second;
                            ...
                            if (rnxver != string("3.03"))
                            {
                                ...
                                if (_spdlog)
                                    SPDLOG_LOGGER_WARN(_spdlog, "Warning: not RINEX observation file version 3.03");
                                rnxver = "3.03";
                            }
                            ...
                            const base_time gtime = rnxHdr.gtime();
                            string theDate = gtime.str("%Y%m%d %H%M%S %T");
                            string::size_type locationOfLOC = theDate.find("LOC");
                            if (locationOfLOC != string::npos)
                                theDate.replace(locationOfLOC, 3, "LCL");
                            ...
                            const Triple aprxyz = rnxHdr.aprxyz();
                            const double aprPosX = aprxyz[0];
                            const double aprPosY = aprxyz[1];
                            const double aprPosZ = aprxyz[2];
                            const string aprPosXstr = base_type_conv::trim(base_type_conv::dbl2str(aprPosX, 4));
                            const string aprPosYstr = base_type_conv::trim(base_type_conv::dbl2str(aprPosY, 4));
                            const string aprPosZstr = base_type_conv::trim(base_type_conv::dbl2str(aprPosZ, 4));
                            ...
                            const Triple antneu = rnxHdr.antneu();
                            ...
                            const string antneuHstr = base_type_conv::trim(base_type_conv::dbl2str(antneuH, 4));
                            const string antneuEstr = base_type_conv::trim(base_type_conv::dbl2str(antneuE, 4));
                            const string antneuNstr = base_type_conv::trim(base_type_conv::dbl2str(antneuN, 4));
                            ...
                            {
                                const Triple ecc = _pcoecc[i];
                                const string eccNX = base_type_conv::trim(base_type_conv::dbl2str(ecc[0], 4));
                                const string eccEY = base_type_conv::trim(base_type_conv::dbl2str(ecc[1], 4));
                                const string eccUZ = base_type_conv::trim(base_type_conv::dbl2str(ecc[2], 4));
                            ...
                            // write SYS / # / OBS TYPES
                            gnss_data_rnxhdr::hwa_map_obs mapobs = rnxHdr.mapobs();
                            gnss_data_rnxhdr::hwa_map_obs::iterator mapobsIt;
                            // for each satellite system
                            for (mapobsIt = mapobs.begin(); mapobsIt != mapobs.end(); ++mapobsIt)
                            {
                                const string satSys = mapobsIt->first;
                                gnss_data_rnxhdr::hwa_vector_obs obsDescriptorFactorPairs = mapobsIt->second;
                                ...
                                gnss_data_rnxhdr::hwa_vector_obs::iterator odfpIt;
                                     ...
                                     ++odfpIt)
                                {
                                    ...
                                    gnss_data_obs gobs_type(gobs);
                                    gobs_type.gobs2to3(gnss_sys::str2gsys(satSys));
                                    ...
                                    if (howManyWrittenOnLine == 13 || howManyWrittenTotal == numObsTypes)
                                    {
                                        ...
                                        if (howManyWrittenTotal != numObsTypes)
                                            _ss << "      ";
                            ...
                            if (!double_eq(interval, 0))
                            {
                                ...
                                const string intervalStr = base_type_conv::trim(base_type_conv::dbl2str(interval, 3));
                                _ss << string(10 - intervalStr.size(), ' ') << intervalStr
                                    << string(50, ' ') << "INTERVAL" << endl;
                            }

                            // write TIME OF FIRST OBS
                            base_time first = rnxHdr.first();
                            ...
                            string secdsecStr = base_type_conv::trim(base_type_conv::dbl2str(((double)secondFO) + dsec, 7));
                            ...
                            if (last != LAST_TIME)
                            {
                                ...
                                string secdsecStrLO = base_type_conv::trim(base_type_conv::dbl2str(((double)secondLO) + dsecLO, 7));
                            ...
                            for (mapobsIt = mapobs.begin(); mapobsIt != mapobs.end(); ++mapobsIt)
                            {
                                ...
                                if (extractedFactors.size() == 1 && extractedFactors.count(1) == 1)
                                {
    ...
    // RINEX encode
    // ---------
    int gnss_coder_rinexo::encode_data(char *buff, int sz, int &cnt, vector<string> &errmsg)
    {
        ...
        if (_ss_position == 0)
        {
            map<string, base_data *>::iterator it = _data.begin();
            //string markname = "";
            gnss_data_rnxhdr::hwa_map_obs mapobs;
            ...
            for (it = _data.begin(); it != _data.end(); ++it)
            {
                if (it->second->id_type() == base_data::ALLOBJ)
                {
                    ...
                    if (allGrec.find(_encode_site) != allGrec.end())
                    {
                        std::shared_ptr<gnss_data_rec> grecCasted = std::dynamic_pointer_cast<gnss_data_rec>(allGrec[_encode_site]);
                        gnss_data_rec::hwa_map_hdr hdrFromGrec = grecCasted->gethdr();
                        if (hdrFromGrec.begin() != hdrFromGrec.end())
                        {
                            gnss_data_rnxhdr rnxHdr = hdrFromGrec.begin()->second;
            ...
            for (it = _data.begin(); it != _data.end(); ++it)
            {
                ...
                {
                    gnss_all_obs *all_obs = (gnss_all_obs *)it->second;
                    ...
                    if (allStations.find(_encode_site) == allStations.end())
                        continue;
                    string markname = _encode_site;

                    vector<base_time> epochs = all_obs->epochs(markname);
                    vector<base_time>::iterator epochsIT = epochs.begin();
            ...
            * that holds the obs, lli, sig strength trios */
                        map<string, shared_ptr<gnss_data_obs_manager>> obsRecords = all_obs->find(markname, *epochsIT);
                        ...
                        string secdsecStr = base_type_conv::trim(base_type_conv::dbl2str(((double)secondEP) + dsec, 7));
                        _ss << '>' << ' ' << yearEP
                            << ' ' << setfill('0') << setw(2) << monthEP
                            << ' ' << setfill('0') << setw(2) << doMEP
                            << ' ' << setfill('0') << setw(2) << hourEP
                            << ' ' << setfill('0') << setw(2) << minuteEP
                     ...
                     */
                            << '0'
                            << setfill(' ') << setw(3) << obsRecords.size() << endl;

                        // for every OBSERVATION record (which means for every line)
                        map<string, shared_ptr<gnss_data_obs_manager>>::iterator obsRecIT;
                        for (obsRecIT = obsRecords.begin(); obsRecIT != obsRecords.end(); ++obsRecIT)
                        {
                            string satNum = obsRecIT->first;
                            _ss << satNum;
                            shared_ptr<gnss_data_obs_manager> currObsRec = obsRecIT->second;
                            gnss_data_rnxhdr::hwa_vector_obs obsDescrs = mapobs[satNum.substr(0, 1)];
                            gnss_data_rnxhdr::hwa_vector_obs::iterator obsDesIT;
                            ...
                            for (obsDesIT = obsDescrs.begin(); obsDesIT != obsDescrs.end(); ++obsDesIT)
                            {
                                ...
                                string currObsStr = base_type_conv::trim(base_type_conv::dbl2str(currObs, 3));
                                _ss << string(14 - currObsStr.size(), ' ') << currObsStr;

                                // now need lli and sig strength
                                // same condition as in gnss_coder_rinexo2::_read_obs
                                    ...
                                    !(GOBS(currObsDescr) >= 1300 && GOBS(currObsDescr) < 1400))
                                {
                                    ...
                                    // again, same condition as in gnss_coder_rinexo2::_read_obs
                                    if (GOBS(currObsDescr) >= 100 && GOBS(currObsDescr) < 200)
                                    {
                                        GOBS snrtype = pha2snr(currObsDescr);

                                        double currSigStrengthConverted = currObsRec->getobs(snrtype);
    ...
    //   - read block of lines at once (tmpsize cummulated and then consumed)
    // ----------
    int gnss_coder_rinexo::_decode_head()
    {
        ...
        while (_complete && ((_tmpsize = gnss_base_coder::_getline(_line)) >= 0))
        {
...
#ifdef DEBUG
            std::cout << "LINE: " << _tmpsize << " " << _consume << " :" << _line;
            std::cout.flush();
#endif
            ...
            { // first line
                ...
                _version = base_type_conv::trim(_line.substr(0, 9)); // F9.2
            ...
            else if (_line.find("END OF HEADER", 60) != string::npos)
            {
                ...
                if (_version < "3.00")
                    ...
                    gnss_coder_rinexo3::_check_head();
                return (_consume = -1);
            }

            else if (_version < "3.00")
                gnss_coder_rinexo2::_decode_head();
            else if (_version < "4.00")
                gnss_coder_rinexo3::_decode_head();
...
#ifdef DEBUG
            if (_complete)
                std::cout << "consume(tmp,rinex::head): " << std::fixed << setw(3) << _tmpsize
                     << " " << right << setw(3) << _endpos
                     << ": " << _line;
            std::cout.flush();
            else std::cout << "not a full record read" << _tmpsize
                      << " tmpsize: " << _tmpsize << endl
                      << " consume: " << _consume << endl;
            std::cout.flush();
#endif
...
#ifdef DEBUG
        std::cout << "consume(tot,rinex::head): " << _consume << " [" << _endpos << "]\n";
        std::cout.flush();
#endif
    ...
    //   - read block of lines for each epoch (tmpsize cummulated and then consumed)
    // ----------
    int gnss_coder_rinexo::_decode_data()
    {
        ...
        while (_complete && ((_tmpsize = gnss_base_coder::_getline(_line)) >= 0))
        {
            ...
            // -------- DECODE DATA -------
            if (_version < "3.00")
                gnss_coder_rinexo2::_decode_data();
            else if (_version < "4.00")
                gnss_coder_rinexo3::_decode_data();

        } // loop over lines

#ifdef DEBUG
        std::cout << "consume(tot,rinex::data): " << _consume << endl;
        std::cout.flush();
#endif
