...
namespace hwa_gnss
{
    gnss_base_celaunay::gnss_base_celaunay()
    {
    }
    ...
    int gnss_base_celaunay::getIntSite(Triple &x_r, vector<Triple> &posList, int outIndx[3])
    {
        if (posList.size() < 3)
            return 0; //zuox

        MESH mesh;           //TODO COMMENT
        point_float Point; //TODO COMMENT
        points3_ABC Point_out;
        //TODO COMMENT
        // path="1.txt";  //TODO COMMENT
        Input_pos(posList, &mesh);    //TODO COMMENT
        IncrementalDelaunay(&mesh); //TODO COMMENT

        Point.x = x_r[0]; //TODO COMMENT
        Point.y = x_r[1]; //TODO COMMENT
        Point_out = Output(&mesh, Point);
        //TODO COMMENT

        //TODO COMMENT
        if (Point_out.A.indx == -1 || Point_out.B.indx == -1 || Point_out.C.indx == -1)
            return 0;

        outIndx[0] = Point_out.A.indx;
        outIndx[1] = Point_out.B.indx;
        outIndx[2] = Point_out.C.indx;
    ...
    // 1 1 0
    // 1 -1 0
    void gnss_base_celaunay::Input(char *pFile, MESH_PTR pMesh)
    {
        ...
        for (int j = 3; j < amount + 3; ++j)
        {
            ...
            //    strcpy (((VERTEX2D_PTR)(pMesh->pVerArr + j))->station , station);
    ...
    void gnss_base_celaunay::Input_pos(vector<Triple> &posList, MESH_PTR pMesh)
    {
        //TODO COMMENT
        int amount;
        ...
        if (amount < 3)
        {
            //fprintf(stderr, "Error:vertex amount should be greater than 2, but it is %d \n", amount);
            //exit(1);
            printf("Error:vertex amount should be greater than 2, but it is %d \n", amount);
            //getchar();
            return;
        }
        ...
        for (int j = 3; j < amount + 3; ++j)
        {
            ...
            double pos_xyz[3] = {0, 0, 0};
            pos_xyz[0] = posList[j - 3][0];
            pos_xyz[1] = posList[j - 3][1];
            pos_xyz[2] = posList[j - 3][2];
            //double pos_blh[3] = { 0,0,0 };
            //pos_blh[0] = posList[j - 3][0];
            //pos_blh[1] = posList[j - 3][1];
            //pos_blh[2] = posList[j - 3][2];
            //ell2xyz(pos_blh, pos_xyz, false);//rad
            /*if (augs_in[j - 3].index == 11)
            {
            pos_xyz[0] = 0;
            pos_xyz[1] = 0;
            }*/
            ((VERTEX2D_PTR)(pMesh->pVerArr + j))->x = pos_xyz[0];
            ((VERTEX2D_PTR)(pMesh->pVerArr + j))->y = pos_xyz[1];
            /*((VERTEX2D_PTR)(pMesh->pVerArr + j))->x = augs_in[j-3].pos[0];
            ((VERTEX2D_PTR)(pMesh->pVerArr + j))->y = augs_in[j - 3].pos[1];*/
            ((VERTEX2D_PTR)(pMesh->pVerArr + j))->indx = j - 3;
    ...
    void gnss_base_celaunay::IncrementalDelaunay(MESH_PTR pMesh)
    {
    ...
    points3_ABC gnss_base_celaunay::Output(MESH_PTR pMesh, point_float Point)
    {
        point_float A, B, C;
        points3_ABC ABC;
        ...
        while (pTri != NULL)
        {
            ...
            {
                vertex_index = *pi++;
                tempX[j] = ((VERTEX2D_PTR)(pMesh->pVerArr + vertex_index))->x;
                tempY[j] = ((VERTEX2D_PTR)(pMesh->pVerArr + vertex_index))->y;
                tempIndx[j] = ((VERTEX2D_PTR)(pMesh->pVerArr + vertex_index))->indx;
            }
            //strcpy(A.station, ((VERTEX2D_PTR)(pMesh->pVerArr + vertex_index))->station);
        ...
        UnInitMesh(pMesh);
        printf("Be careful! The point is not in Triangle\n");
    ...
    void gnss_base_celaunay::InitMesh(MESH_PTR pMesh, int ver_num)
    {
    ...
    // Deallocate memory
    void gnss_base_celaunay::UnInitMesh(MESH_PTR pMesh)
    {
        // free vertices
        if (pMesh->pVerArr != NULL)
            free(pMesh->pVerArr);
    ...
    void gnss_base_celaunay::AddBoundingBox(MESH_PTR pMesh)
    {
        double max = 0;
        double max_x = 0;
        double max_y = 0;
    ...
    void gnss_base_celaunay::RemoveBoundingBox(MESH_PTR pMesh)
    {
        int statify[3] = {0, 0, 0};
        int vertex_index;
        int *pi;
    ...
    double gnss_base_celaunay::CounterClockWise(VERTEX2D_PTR pa, VERTEX2D_PTR pb, VERTEX2D_PTR pc)
    {
        return ((pb->x - pa->x) * (pc->y - pb->y) - (pc->x - pb->x) * (pb->y - pa->y));
    }

    // Adjust if the point lies in the triangle abc
    double gnss_base_celaunay::InTriangle(MESH_PTR pMesh, VERTEX2D_PTR pVer, TRIANGLE_PTR pTri)
    {
        int vertex_index;
    ...
    void gnss_base_celaunay::Insert(MESH_PTR pMesh, int ver_index)
    {
    ...
    void gnss_base_celaunay::InsertInTriangle(MESH_PTR pMesh, TRIANGLE_PTR pTargetTri, int ver_index)
    {
    ...
    void gnss_base_celaunay::InsertOnEdge(MESH_PTR pMesh, TRIANGLE_PTR pTargetTri, int ver_index)
    {
        int index_a, index_b, index_c;
        TRIANGLE_PTR pTri = NULL;
        TRIANGLE_PTR pNewTri = NULL;
    ...
    bool gnss_base_celaunay::FlipTest(MESH_PTR pMesh, TRIANGLE_PTR pTestTri)
    {
        bool flipped = false;
    ...
    double gnss_base_celaunay::InCircle(VERTEX2D_PTR pa, VERTEX2D_PTR pb, VERTEX2D_PTR pp, VERTEX2D_PTR pd)
    {
        double det;
    ...
    void gnss_base_celaunay::RemoveTriangleNode(MESH_PTR pMesh, TRIANGLE_PTR pTri)
    {
        if (pTri == NULL)
        {
            return;
    ...
    TRIANGLE_PTR gnss_base_celaunay::AddTriangleNode(MESH_PTR pMesh, TRIANGLE_PTR pPrevTri, int i1, int i2, int i3)
    {
    ...
    //TODO COMMENT
    double gnss_base_celaunay::GetTriangleSquar(point_float pf0, point_float pf1, point_float pf2)
    {
        point_float AB, BC;
        AB.x = pf1.x - pf0.x;
        AB.y = pf1.y - pf0.y;
    ...
    //TODO COMMENT
    int gnss_base_celaunay::IsInTriangle(point_float A, point_float B, point_float C, point_float D)
    {
        double SABC, SADB, SBDC, SADC, Sum;
        ...
        if (hwa_base::double_eq(SABC - Sum, 0))
        {
            return 1;
        }
        else
