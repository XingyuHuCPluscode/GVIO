...
namespace hwa_gnss
{
    ...
    double gnss_model_gmf::ah_mean[55] = {
    ...
    double gnss_model_gmf::bh_mean[55] = {
    ...
    double gnss_model_gmf::ah_amp[55] = {
    ...
    double gnss_model_gmf::bh_amp[55] = {
    ...
    double gnss_model_gmf::aw_mean[55] = {
    ...
    double gnss_model_gmf::bw_mean[55] = {
    ...
    double gnss_model_gmf::aw_amp[55] = {
    ...
    double gnss_model_gmf::bw_amp[55] = {
    ...
    // ----------
    int gnss_model_gmf::gmf(double dmjd, double dlat, double dlon, double dhgt, double zd,
                   double &gmfh, double &gmfw, double &dgmfh, double &dgmfw)
    {
        ...
        double ch = c0h + ((cos(doy / 365.25 * 2.0 * hwa_pi + phh) + 1.0) * c11h / 2.0 + c10h) * (1.0 - cos(dlat));
        ...
        double ah = (ahm + aha * cos(doy / 365.25 * 2.0 * hwa_pi)) * 1e-5;

        // mapping function and its derivative (before height correction)
        double sine = sin(hwa_pi / 2 - zd);
        double cose = cos(hwa_pi / 2 - zd);
        ...
        // std::cout << " height = " << hs_km << "  zd = " << zd/hwa_pi*180 << " ht_corr = " << ht_corr << endl;
        gmfh += ht_corr * hs_km;
        ...
        double aw = (awm + awa * cos(doy / 365.25 * 2.0 * hwa_pi)) * 1e-5;
